...
- Run `liberate_compare({ originDir: "output/<site>/screenshots", replicaDir: "output/<site>/replica-carry" })` against the SOURCE screenshots. Emit `output/<site>/run-report-carry.json` with a per-page and overall parity score, mirroring the block path's `run-report.json` shape. **Include `residualCdnAssets`** (from the step-3 driver's self-host audit; 0 = fully self-hosted) with any offending hosts/samples — this is a correctness deliverable, reported separately from the pixel score.
