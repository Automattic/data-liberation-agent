...
if (args[0] === 'mcp') {
  ...
  Liberate options:
    ...
    --learn-fluid        Sweep viewport widths and learn how the source sizes itself,
                         so the copy keeps reflowing instead of freezing at one width
...
`);
  ...
  const result = await liberateSite({
    ...
    learnFluid: args.includes('--learn-fluid'),
