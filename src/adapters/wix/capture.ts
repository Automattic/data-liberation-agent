...
/**
 * Replaces Wix's one-mounted-slide runtime state with every distinct state
 * observed through its authored next control. The captured document remains
 * static, but all source content is available to downstream transformation.
 */
...
function slideSnapshot( page: Page, slideshowIndex: number ): Promise< { html: string; key: string } | null > {
...
export async function collectWixSlideshowSlides( page: Page ): Promise< void > {
	...
	for ( let slideshowIndex = 0; slideshowIndex < Math.min( count, WIX_SLIDESHOW_LIMIT ); slideshowIndex++ ) {
		...
		for ( let step = 0; step < WIX_SLIDE_LIMIT; step++ ) {
			const snapshot = await slideSnapshot( page, slideshowIndex );
		...
		if ( slides.length > 1 ) {
			// Keep this callback self-contained: Playwright serializes it into the
			// source page, where module bindings are not available.
			await page.evaluate( ( { slideshowIndex, slides } ) => {
				const slideshow = document.querySelectorAll< HTMLElement >( '.wixui-slideshow' )[ slideshowIndex ];
				const wrapper = slideshow?.querySelector< HTMLElement >( '[data-testid="slidesWrapper"]' );
				if ( ! slideshow || ! wrapper ) return;
				const fragment = document.createDocumentFragment();
				for ( const [ index, html ] of slides.entries() ) {
					const template = document.createElement( 'template' );
					template.innerHTML = html;
