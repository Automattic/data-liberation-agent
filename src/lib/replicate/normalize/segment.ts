...
/** Stable id: existing id → first class → heading-text slug → content hash. */
function stableId($: CheerioAPI, el: Element, ordinal: number): string {
  ...
  if (heading.trim()) return textSlug(heading);
...
export function segmentPage(html: string): Section[] {
  ...
  pushChrome('body > header, header', 'header');
  pushChrome('body > footer, footer', 'footer');
