...
- Screenshots live at `output/<site>/screenshots/{desktop,mobile}/<slug>.png` and `<slug>.scrolled.png` (post-scroll viewport capture). Rendered HTML is at `output/<site>/html/<slug>.html`. Screenshot-to-WXR joining happens via two mechanisms: (1) `output/<site>/screenshots/manifest.json` maps URL → files (always present); (2) when `--screenshots` runs with extract, WXR posts/pages get `_liberation_screenshot_desktop`, `_liberation_screenshot_mobile`, `_liberation_screenshot_desktop_scrolled`, `_liberation_screenshot_mobile_scrolled`, `_liberation_html` postmeta, and products get matching `meta:_liberation_*` CSV columns. Shared constant: `src/lib/screenshot/meta-keys.ts`.
- The screenshot feature introduces two `ImportSession` stages: `screenshotting` (capture loop) and `stamping-metadata` (post-capture WXR/CSV rewrite). Both are gated behind the `--screenshots` opt-in.
